...
final class CapabilityRepository
{
    ...
    {
        global $wpdb;
        $table = Schema::capabilities_table();
        $wpdb->insert($table, [
    ...
    public function findCapability(int $applicationId, string $capability): ?array
    {
        ...
        $row = $wpdb->get_row($wpdb->prepare(
            'SELECT * FROM ' . Schema::capabilities_table() . ' WHERE application_id = %d AND capability = %s LIMIT 1',
            $applicationId,
            $capability
        ), ARRAY_A);
    ...
    public function grant(int $applicationId, int $userAccountId, int $capabilityId): void
    {
        ...
        $wpdb->query($wpdb->prepare(
            'INSERT IGNORE INTO ' . Schema::user_capabilities_table() . ' (application_id, user_account_id, capability_id, created_at) VALUES (%d, %d, %d, %s)',
            $applicationId, $userAccountId, $capabilityId, gmdate('Y-m-d H:i:s')
        ));
    }
    ...
    {
        global $wpdb;
        $wpdb->delete(Schema::user_capabilities_table(), [
            'application_id' => $applicationId,
