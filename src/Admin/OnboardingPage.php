...
final class OnboardingPage
{
    ...
    public static function menu(): void
    {
        foreach (self::$contexts as $context) {
            if (!self::needsOnboarding($context)) continue;
            add_submenu_page(
                ...
                $context->capability,
                $context->menuSlug,
                static function () use ($context): void { self::renderContext($context); }
            );
        }
    }
    ...
    public static function handle(): void
    {
        ...
        if (!$context instanceof OnboardingContext) wp_die(esc_html__('Invalid onboarding context.', 'authcore'));
        if (!current_user_can($context->capability)) wp_die(esc_html__('You do not have permission to access this page.', 'authcore'));
        check_admin_referer('authcore_onboarding_' . $context->menuSlug);
        if (self::hasUsers($context)) self::redirect($context, 'already_initialized');
        ...
        if ($password !== $passwordConfirmation) self::redirect($context, 'password_mismatch');
    ...
    private static function renderContext(OnboardingContext $context): void
    {
        ...
        echo '<div class="wrap"><h1>' . esc_html($context->pageTitle) . '</h1>';
        ...
        if ($message === 'password_mismatch') echo '<div class="notice notice-error"><p>' . esc_html__('The passwords do not match.', 'authcore') . '</p></div>';
        if ($message === 'error') echo '<div class="notice notice-error"><p>' . esc_html__('The administrator account could not be created. Please check the values and try again.', 'authcore') . '</p></div>';
        if ($message === 'already_initialized') echo '<div class="notice notice-info"><p>' . esc_html__('Initial administrator setup has already been completed.', 'authcore') . '</p></div>';

        echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '">';
        wp_nonce_field('authcore_onboarding_' . $context->menuSlug);
        echo '<input type="hidden" name="action" value="authcore_onboarding"><input type="hidden" name="authcore_onboarding_slug" value="' . esc_attr($context->menuSlug) . '">';
    ...
    private static function needsOnboarding(OnboardingContext $context): bool { return !self::hasUsers($context); }
    private static function hasUsers(OnboardingContext $context): bool { return AuthCore::listUserAccounts($context->applicationId, 1, 0) !== []; }

    private static function redirect(OnboardingContext $context, string $message): void
    {
        $url = add_query_arg(['page' => $context->menuSlug, 'authcore_onboarding_message' => $message], admin_url('admin.php'));
