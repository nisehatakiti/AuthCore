...
final class UserManagementContext
{
    ...
    public static function create(
        ...
        if ($parentMenuSlug === '' || $menuTitle === '' || $pageTitle === '' || $capability === '') {
            throw new AuthCoreException('Invalid AuthCore user management menu configuration.');
        }
        ...
        return new self(
            $contextKey,
            $applicationId,
            'authcore-users-' . substr(hash('sha256', $contextKey), 0, 16),
            $parentMenuSlug,
            $menuTitle,
            $pageTitle,
            $capability,
